...
class KittiDataset:
    def __init__(self, dataset_config):
        ...
        self.name = self.config.name                  # "kitti"
        self.data_split = self.config.data_split      # "trainval"
        self.dataset_dir = os.path.expanduser(self.config.dataset_dir)  # /home/yxk/Kitti/object
        data_split_dir = self.config.data_split_dir   # "training"

        self.has_labels = self.config.has_labels      # true
        self.cluster_split = self.config.cluster_split  # "train"

        self.classes = list(self.config.classes)   # "car"
        self.num_classes = len(self.classes)
        self.num_clusters = np.asarray(self.config.num_clusters)  # 2

        self.bev_source = self.config.bev_source  # "lidar"
        ...
        all_files = os.listdir(self.dataset_dir)    # <class 'list'>: ['trainval.txt', 'train.txt', 'val.txt', 'testing', 'training']

        # Get possible data splits from txt files in dataset folder
        possible_splits = []
        for file_name in all_files:
            if fnmatch.fnmatch(file_name, '*.txt'):
                possible_splits.append(os.path.splitext(file_name)[0])   # <class 'list'>: ['trainval', 'train', 'val']
                # print(os.path.splitext(file_name))
        # print(possible_splits)  ['trainval', 'train', 'val']
        ...
        for folder_name in all_files:
            if os.path.isdir(self.dataset_dir + '/' + folder_name):
                possible_split_dirs.append(folder_name)         # <class 'list'>: ['testing', 'training']
        if data_split_dir in possible_split_dirs:
            split_dir = self.dataset_dir + '/' + data_split_dir
            self._data_split_dir = split_dir    # /home/yxk/Kitti/object/training
            # print(split_dir)   # /home/yxk/Kitti/object/training
        else:
            raise ValueError(
        ...
        loaded_sample_names = self.load_sample_names(self.data_split)  # 加载数据name

        # Augment the sample list
        aug_sample_list = []

        # Loop through augmentation lengths e.g.  数据增强
        # 0: []
        # 1: ['flip'], ['pca_jitter']
        # 2: ['flip', 'pca_jitter']
        ...
        for aug_idx in range(len(self.aug_list) + 1):
            ...
            for augmentation in augmentations:
                for sample_name in loaded_sample_names:
                    aug_sample_list.append(Sample(sample_name, augmentation))  # name 000000   <class 'tuple'>: ()

        self.sample_list = np.asarray(aug_sample_list)  # 7481 name augs
        self.num_samples = len(self.sample_list)        # 7481
    ...
    def load_samples(self, indices):
        ...
        for sample_idx in indices:
            ...
            sample_dict = {
                constants.KEY_LABEL_BOXES_3D: label_boxes_3d,  # (2, 7)

                constants.KEY_LABEL_ANCHORS: label_anchors,    # (2, 6)

                constants.KEY_LABEL_CLASSES: label_classes,    # 2

                constants.KEY_IMAGE_INPUT: image_input,        # <class 'tuple'>: (375, 1242, 3)
                constants.KEY_BEV_INPUT: bev_input,            # <class 'tuple'>: (700, 800, 6)

                constants.KEY_ANCHORS_INFO: anchors_info,      # 4 个(21774,)

                constants.KEY_POINT_CLOUD: point_cloud,
                constants.KEY_GROUND_PLANE: ground_plane,        # 4
                constants.KEY_STEREO_CALIB_P2: stereo_calib_p2,  # (3, 4)
