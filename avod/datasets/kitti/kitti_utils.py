...
class KittiUtils(object):
    ...
    def __init__(self, dataset):  # dataset <avod.datasets.kitti.kitti_dataset.KittiDataset object at 0x7f0e90f900f0>
                                  # self    <avod.datasets.kitti.kitti_utils.KittiUtils object at 0x7f0e315578d0>
        self.dataset = dataset

        # Label Clusters
        self.label_cluster_utils = LabelClusterUtils(self.dataset)  # <avod.core.label_cluster_utils.LabelClusterUtils object at 0x7f0e315570b8>

        self.clusters, self.std_devs = [None, None]

        # BEV source from dataset config
        self.bev_source = self.dataset.bev_source   # 'lidar'

        # Parse config
        self.config = dataset.config.kitti_utils_config
        self.area_extents = np.reshape(self.config.area_extents, (3, 2))   # [-40, 40, -5, 3, 0, 70]
        self.bev_extents = self.area_extents[[0, 2]]                       # [[-40.  40.] [ -5.   3.] [  0.  70.]]
        self.voxel_size = self.config.voxel_size                           # 0.1
        self.anchor_strides = np.reshape(self.config.anchor_strides, (-1, 2))   # anchor_strides: [0.5, 0.5]

        self.bev_generator = bev_generator_builder.build(
            self.config.bev_generator, self)                # height_lo: -0.2
                                                            # height_hi: 2.3
                                                            # num_slices: 5

        self._density_threshold = self.config.density_threshold      # 1
        ...
        self._mini_batch_dir = self.mini_batch_utils.mini_batch_dir    # '/home/yxk/project/avod/avod/data/mini_batches/iou_2d/kitti/train/lidar'
    ...
    def create_sliced_voxel_grid_2d(self, sample_name, source,
        ...
        ground_plane = obj_utils.get_road_plane(img_idx,
                                                self.dataset.planes_dir)   # planes 读取的数据 规范化

        point_cloud = self.get_point_cloud(source, img_idx,
                                           image_shape=image_shape)        # 图像范围内的点,摄像头前前面的点
        filtered_points = self._apply_slice_filter(point_cloud, ground_plane)  # 高度上过滤一遍
        ...
        voxel_grid_2d.voxelize_2d(filtered_points, self.voxel_size,
                                  ...
                                  create_leaf_layout=True)    # 产生voxel grid
