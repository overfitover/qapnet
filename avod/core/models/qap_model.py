import numpy as np
import sys
sys.path.append('/home/yxk/project/avod')
sys.path.append('/home/yxk/project/avod/wavedata')
...
class qapModel(model.DetectionModel):
    ##############################
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        super(qapModel, self).__init__(model_config)

        self.dataset = dataset

        # Dataset config
        self._num_final_classes = self.dataset.num_classes + 1           # 2
        ...
        # print(self._bev_pixel_size,  '\n', self._bev_depth, '\n', self._img_pixel_size, '\n', self._img_depth)

        # AVOD config
        avod_config = self._config.avod_config
        self._proposal_roi_crop_size = \
            [avod_config.avod_proposal_roi_crop_size] * 2               # [7, 7]
        self._positive_selection = avod_config.avod_positive_selection  # not_bkg
        self._nms_size = avod_config.avod_nms_size                      # 100
        self._nms_iou_threshold = avod_config.avod_nms_iou_thresh       # 0.01
        self._path_drop_probabilities = self._config.path_drop_probabilities  # [0.9, 0.9]
        self._box_rep = avod_config.avod_box_representation                   # box_4ca

        # print(self._proposal_roi_crop_size)
        # print(self._positive_selection)
        # print(self._nms_size)
        # print(self._nms_iou_threshold)
        # print(self._path_drop_probabilities)
        # print(self._box_rep)
