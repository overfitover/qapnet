...
def main():
    ...
    clusters, _ = label_cluster_utils.get_clusters()   # <class 'list'>: [[array([3.892, 1.619, 1.53 ])]]
    ...
    anchor_boxes_3d = anchor_3d_generator.generate(                    # <class 'tuple'>: (448, 7)
        area_3d=dataset.kitti_utils.area_extents,
        anchor_3d_sizes=fake_clusters,                                 # anchor 尺寸
        anchor_stride=fake_anchor_stride,                              # 采样步长
        ground_plane=ground_plane)
    all_anchors = box_3d_encoder.box_3d_to_anchor(anchor_boxes_3d)     # <class 'tuple'>: (448, 6)  考虑到object倾斜的情况,这样边的长度会相对减小
    end_time = time.time()
    print("Anchors generated in {} s".format(end_time - start_time))

    # Project into bev
    bev_boxes, bev_normalized_boxes = \
        anchor_projector.project_to_bev(all_anchors, area_extents[[0, 2]])     # <class 'tuple'>: (448, 4)  <class 'tuple'>: (448, 4)
    ...
    for box in bev_boxes:        # x1,y1,x2,y2
        box_w = box[2] - box[0]
        box_h = box[3] - box[1]
    ...
    for anchor_idx in range(len(anchor_boxes_3d)):
        anchor_box_3d = anchor_boxes_3d[anchor_idx]       # (7,)
