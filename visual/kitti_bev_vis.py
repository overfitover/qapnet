import sys
sys.path.append('/home/ovo/project/graduation_project/qapNet')
sys.path.append('/home/ovo/project/graduation_project/qapNet/wavedata')
# print(sys.path)
...
def main():
    ...
    dataset_config = DatasetBuilder.merge_defaults(dataset_config)         # 合并默认参数与DatasetBuilder.KITTI_VAL

    # Overwrite bev_generator
    if bev_generator == 'slices':
        text_format.Merge(slices_config,
                          dataset_config.kitti_utils_config.bev_generator)  # 将slices_config传给第二个参数
    else:
        raise ValueError('Invalid bev_generator')

    # print(dataset_config)  合并了dataset_config

    dataset = DatasetBuilder.build_kitti_dataset(dataset_config,
                                                 use_defaults=False)  # 使用配置文件
    ...
    point_cloud = kitti_utils.get_point_cloud(
        point_cloud_source, img_idx, image_shape)                         # (3, n) 返回点云数据
    ground_plane = kitti_utils.get_ground_plane(sample_name)              # (4)
    bev_images = kitti_utils.create_bev_maps(point_cloud, ground_plane)   # (700, 800, 6)
    ...
    if show_ground_truth:
        ...
        box_points, box_points_norm = box_3d_projector.project_to_bev(
            label_boxes, [[-40, 40], [0, 70]])     # (3, 4, 2)

    rgb_img_size = (np.array((1242, 375)) * 0.75).astype(np.int16)       # [931, 281]
    img_x_start = 60
    img_y_start = 330
