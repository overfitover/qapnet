...
def split_indices(dataset, num_children):
    ...
    # print("indices_split", np.shape(indices_split))  # (8,)
...
def split_work(all_child_pids, dataset, indices_split, num_children):
    ...
    """
    # print(num_children)
    for child_idx in range(num_children):   # num_children: 8
        new_pid = os.fork()
        # print(new_pid)
        if new_pid:       # 父进程返回子进程的pid   子进程返回0
            all_child_pids.append(new_pid)
        else:
            indices = indices_split[child_idx]   # indices: 7481
